-- Initial schema for the webhook delivery service.
...
CREATE TABLE endpoints (
  ...
  -- Halted-endpoint ticket: unifies R-4 pause/resume and R-14 halt-at-ceiling
  -- as one concept rather than two independent flags.
  status            TEXT NOT NULL DEFAULT 'active'
                      CHECK (status IN ('active', 'paused', 'halted')),
  -- Encrypted at rest (AES-256-GCM, node/src/lib/crypto.ts), not plaintext
  ...
  -- ADR-0003 (docs/adr/): during a rotation overlap window, the sender signs
  -- with BOTH the current secret and this one — not receiver-only dual-check,
  ...
  -- ADR-002 ordering: one delivery in flight per endpoint at a time, enforced
  -- via a short-lived row lock on this row at claim time (not held for the
  -- outbound HTTP call). ADR-003 crash recovery: busy_since turns this into a
  -- lease — a claim older than the configured lease duration is reclaimable.
  busy              BOOLEAN NOT NULL DEFAULT false,
  busy_since        TIMESTAMPTZ,
  -- ADR-0002 (docs/adr/): fencing token set fresh at claim time, alongside
...
CREATE TABLE events (
  ...
  -- ADR-004 async expansion: R-9 (don't let a customer mistake zero
  -- deliveries-so-far for dropped) is satisfied by this explicit status,
  -- not by hoping expansion stays fast.
  status            TEXT NOT NULL DEFAULT 'pending_expansion'
                      CHECK (status IN ('pending_expansion', 'expanded')),
  -- ADR-0001 (docs/adr/): monotonic publish-order key. Expansion claims a
  -- tenant's oldest pending_expansion event by seq (not just created_at),
  -- serialized per tenant via pg_try_advisory_xact_lock(tenant_id) — this is
  -- what makes per-endpoint delivery order actually match publish order,
  -- since expansion itself is otherwise unordered across concurrent workers.
  seq               BIGSERIAL,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE deliveries (
  ...
  -- doesn't get its own delivery state.
...
-- unresolved delivery — this index is what makes that lookup cheap.
CREATE INDEX idx_deliveries_endpoint_pending ON deliveries (endpoint_id, created_at)
...
CREATE TABLE attempts (
  ...
  duration_ms               INT,
  -- ADR-003: a reclaimed lease closes its orphaned attempt with
...
CREATE TABLE replays (
  ...
  -- ADR-0005 (docs/adr/): replay is async-expanded exactly like events
  -- (ADR-004) — the durable ack is this row alone; a worker later selects
  -- matching original deliveries (excluding still-pending/in_flight ones,
  -- which will be attempted on their own schedule regardless) and creates
  -- the replayed delivery rows in one atomic transaction, then flips this to
  -- 'expanded'. Fixes both the O(window) synchronous-request problem and the
  -- crash-unsafe idempotency gap the original synchronous design had.
  status            TEXT NOT NULL DEFAULT 'pending_expansion'
                      CHECK (status IN ('pending_expansion', 'expanded')),
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
